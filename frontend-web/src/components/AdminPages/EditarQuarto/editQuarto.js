...
function EditarQuarto() {
  ...
  useEffect(() => {
    async function fetchData() {
      const data = await getQuartoById(id);
      if (data) {
        setForm({
          nome: data.nome || '',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await updateQuarto(id, form); // Certifique-se de que `form` contém os dados corretos
      navigate('/admin/quartos'); // Redireciona após salvar
    } catch (error) {
